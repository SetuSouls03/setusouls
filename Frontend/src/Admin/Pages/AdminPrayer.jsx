...
// Default data structure matching prayer model
const DEFAULT_PRAYER_DATA = {
  sections: [
    {
      heading: {
        hi: "प्रार्थना",
...
const AdminPrayer = () => {
  ...
  const [editingData, setEditingData] = useState(DEFAULT_PRAYER_DATA);
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`${API_BASE}/api/prayer`);
      ...
      toast.error("Failed to load prayer content");
      
      // Use default data if fetch fails
      setData(DEFAULT_PRAYER_DATA);
      setEditingData(JSON.parse(JSON.stringify(DEFAULT_PRAYER_DATA)));
    }
  };

  const validateAndNormalizeData = (data) => {
    if (!data) return DEFAULT_PRAYER_DATA;
    
    // Ensure sections exist and is an array
    if (!data.sections || !Array.isArray(data.sections)) {
      return { ...DEFAULT_PRAYER_DATA, ...data, sections: DEFAULT_PRAYER_DATA.sections };
    }
    ...
    setEditingData(prev => {
      ...
      try {
        if (path.includes('sections') && index !== null) {
          ...
          const field = keys[1]; // 'heading' or 'subheading'
  ...
  const addSection = () => {
    setEditingData(prev => {
      ...
      newSections.push({
        ...
        subheading: { hi: "नया सामग्री", en: "New Content" }
      });
  ...
  try {
    ...
    console.log("Attempting to save prayer data:", {
      sectionsCount: validatedData.sections?.length,
      firstSection: validatedData.sections?.[0]
    });
    
    // FIRST: Test if the route exists
    try {
      console.log("Testing prayer API...");
      const testRes = await axios.get(`${API_BASE}/api/prayer/test`);
      console.log("API test result:", testRes.data);
    } catch (testErr) {
      console.error("API test failed:", testErr.message);
      toast.error("⚠️ Prayer API not responding. Please check server.");
      throw testErr;
    }
    
    // SECOND: Try PUT request
    console.log("Sending PUT request to /api/prayer");
    const res = await axios.put(`${API_BASE}/api/prayer`, validatedData);
    ...
    toast.success("✅ Prayer content saved successfully!");
    ...
    if (err.response?.status === 404) {
      toast.error(
        <div>
          ...
          <br />
          <small>Check if prayerRoutes.js has router.put("/", ...)</small>
        </div>,
        { duration: 6000 }
      );
    } else if (err.response?.status === 500) {
      toast.error("❌ Server error. Check Render.com logs.");
  ...
  const exportData = () => {
    ...
    link.href = url;
    link.download = `prayer-backup-${new Date().toISOString().split('T')[0]}.json`;
  ...
  const safeEditingData = editingData || DEFAULT_PRAYER_DATA;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading Prayer CMS...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-purple-100 p-4 md:p-6">
      <Toaster 
        position="top-right"
        toastOptions={{
          duration: 4000,
      ...
      <div className="flex justify-between items-center mb-4 gap-2">
        <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
          <FaHeading className="text-purple-600" /> Prayer Page CMS
        </h1>
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 md:p-6 mb-6">
        ...
        <div className="mt-4">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search in prayer content..."
              value={searchTerm}
                  ...
                  <div className="space-y-6">
                    <div className="flex justify-between items-center">
                      <h3 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
                        <FaListOl className="text-purple-500" /> Prayer Sections
                      </h3>
                      {isEditing && (
                        <button
                          onClick={addSection}
                          ...
                          {expandedSection === index && (
                            <div className="p-4 space-y-4">
                              ...
                              <div>
                                ...
                                <textarea
                                  value={section.subheading?.hi || ""}
                                  onChange={(e) => handleInputChange('sections.subheading.hi', e.target.value, index)}
                                  disabled={!isEditing}
                                  rows="3"
                                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent disabled:bg-gray-100 font-devanagari"
                                />
                              ...
                              <div>
                                ...
                                <textarea
                                  value={section.subheading?.en || ""}
                                  onChange={(e) => handleInputChange('sections.subheading.en', e.target.value, index)}
                                  disabled={!isEditing}
                                  rows="3"
                                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent disabled:bg-gray-100"
                                />
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="space-y-2">
                          ...
                          <input type="color" className="w-full h-10 rounded-lg cursor-pointer" defaultValue="#eef0ff" />
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="p-4 md:p-6">
              <div className={`transition-all duration-300 ${previewMode ? 'scale-100' : 'scale-95'}`}>
                ...
                <div 
                  ...
                  style={{
                    background: `linear-gradient(to bottom, #fefcea, #eef0ff)`
                  }}
                >
                  {/* Preview Content */}
                    ...
                    {safeSections.map((section, index) => (
                      ...
                      >
                        ...
                        <div className="text-center font-semibold text-black leading-[2.6rem] text-[clamp(1.1rem,2.5vw,1.75rem)] space-y-3">
                          {section.subheading && (
                            <div>
                              {section.subheading[language]?.split("\n").map((line, i) => (
                                <p key={i}>{line || "\u00A0"}</p>
                              ))}
                            </div>
                          )}
